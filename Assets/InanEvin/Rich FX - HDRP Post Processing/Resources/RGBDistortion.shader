...
#define off3 _Intensity * 0.7f
    ...
    float4 CustomPostProcess(Varyings input) : SV_Target
    {
		...
		float2 positionSSR = (input.texcoord + offset1) * _PostProcessScreenSize.xy;
		float2 positionSSG = (input.texcoord + offset2) * _PostProcessScreenSize.xy;
		float2 positionSSB = (input.texcoord + offset3) * _PostProcessScreenSize.xy;
