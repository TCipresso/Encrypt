...
#include "Packages/com.unity.render-pipelines.high-definition/Runtime/ShaderLibrary/ShaderVariables.hlsl"
	...
	float4 Fragment(Varyings input) : SV_Target
	{
		...
		const float aspect = (float)_PostProcessScreenSize.x / _PostProcessScreenSize.y;
		const float inv_aspect = (float)_PostProcessScreenSize.y / _PostProcessScreenSize.x;
