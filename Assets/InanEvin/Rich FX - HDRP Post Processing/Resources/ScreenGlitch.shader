...
#define MULTP2 0.85373472095314
	...
	float4 CustomPostProcess(Varyings input) : SV_Target
	{
		...
		float4 fragColor = LOAD_TEXTURE2D_X(_InputTexture, float2(xpos, input.texcoord.y) * _PostProcessScreenSize.xy);
		...
		fragColor.g = lerp(fragColor.r, LOAD_TEXTURE2D_X(_InputTexture, float2(xpos + n * 0.05, input.texcoord.y) * _PostProcessScreenSize.xy).g, _ColorLerp);
		fragColor.b = lerp(fragColor.r, LOAD_TEXTURE2D_X(_InputTexture, float2(xpos - n * 0.05, input.texcoord.y) * _PostProcessScreenSize.xy).b, _ColorLerp);
		return fragColor;
	}
