...
#define RANDSIN 33758.5453
	...
	float4 CustomPostProcess(Varyings input) : SV_Target
	{
		...
		float4 finalColor = float4(LOAD_TEXTURE2D_X(_InputTexture, input.texcoord * _PostProcessScreenSize.xy).rgb, 1.0);
		float4 edges = 1 - (finalColor * _BaseModifier / float4(LOAD_TEXTURE2D_X(_InputTexture, uv * _PostProcessScreenSize.xy).rgb, 1.0));
