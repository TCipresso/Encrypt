...
/* Version: 1.1.38  */
/* Exported on: 27.08.2024 22:12:37 */
		...
		Pass
		{
			...
			inline void MKPostProcessing(in float2 uv, in MKUserData userData, out half4 result)
			{
				half4 mainSample = LoadTex2DAuto(PASS_TEXTURE_2D(_MKShaderMainTex, MK_SAMPLER_LINEAR_CLAMP), uv, _MKShaderMainTexDimension);
					...
					#endif
				#else
					#if defined(MK_FADE) || defined(MK_INPUT_DEPTH) || defined(MK_FOG) || defined(MK_RENDER_PIPELINE_HIGH_DEFINITION)
						...
						*/
						...
						#endif
					#else
						const half combinedInputMask = 0;
					#endif
						...
						result.rgb = lerp(half3(0, 0, 0), half3(1, 1, 1) - half3(_LineHardness, _LineHardness, _LineHardness), half3(depthMask + combinedInputMask, normalMask, sceneColorMask) * fogMult);
